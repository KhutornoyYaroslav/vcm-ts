...
def do_train(cfg,
    ...
    for epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        ...
        # Create progress bar
        print(('\n' + '%12s' * 8) % ('Epoch', 'gpu_mem', 'lr', 'lr_spynet', 'loss', 'charb_loss', 'perc_loss', 'psnr'))
        ...
        for iteration, data_entry in pbar:
            ...
            charb_loss_sum += charb_loss.item()
            percept_loss_sum += percept_loss.item()
            ...
            s = ('%12s' * 2 + '%12.4g' * 6) % ('%g/%g' % (epoch, cfg.SOLVER.MAX_EPOCH - 1),
                                               mem,
                                               optimizers['net'].param_groups[0]['lr'],
                                               optimizers['net'].param_groups[1]['lr'],
                                               loss_sum / (iteration + 1),
                                               charb_loss_sum / (iteration + 1),
                                               percept_loss_sum / (iteration + 1),
                                            #    10 * np.log10(1.0 / (mse_loss_sum / (cfg.SOLVER.BATCH_SIZE*iteration + 1)))
                                               10 * np.log10(1.0 / (mse_loss_sum / (iteration + 1)))
                                               )
